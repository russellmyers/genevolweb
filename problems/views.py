...
def breeders_equation(request):
    ...
    if request.method == 'POST':
        ...
        if 'solverSubmit' in request.POST:
            default_tab = 0
            be = BreedersEquationProblem(
                BreedersEquationProblem.create_solver_form_from_query_params(request, post=True))
            form = be.solver_form
...
def children_stats(children):
    ...
    phen_combinations_per_pair = [{} for _ in range(len(comb_list))]
    ...
    return phenotypes, pairs_cis, dists, parentals, double_crossovers, recomb_fraction_list, pairs_list,\
        phen_combinations_per_pair, gamete_counts_het, recomb_fraction_list_with_p
...
def cross_map(request):
    ...
    gt = create_genome_template(request, chroms_in=chroms_in, positions_in=positions_in,
                                use_specific_genome_name=genome_name_requested)
