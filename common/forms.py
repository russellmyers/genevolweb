...
class HardyWeinbergSolverForm(forms.Form):
    ...
    obs_Aa = forms.IntegerField(label='Observed Aa count', required=False, validators=[MinValueValidator(0, 'Enter positive Aa genotype count')], widget=forms.NumberInput(attrs={'class': 'form-control form-control-sm narrow-select solver-input', 'step':'1'}))
    obs_aa = forms.IntegerField(label='Observed aa count', required=False, validators=[MinValueValidator(0, 'Enter positive aa genotype count')], widget=forms.NumberInput(attrs={'class': 'form-control form-control-sm narrow-select solver-input', 'step':'1'}))
    p      = forms.FloatField(label='Freq A allele (p)', required=False, widget=forms.NumberInput(attrs={'class': 'form-control  form-control-sm narrow-select generator-input',  'min': 0, 'max':'1', 'step':'0.001', 'answer_field': True}))
    q      = forms.FloatField(label='Freq a allele (q)', required=False, widget=forms.NumberInput(attrs={'class': 'form-control  form-control-sm narrow-select generator-input',  'min': 0, 'max':'1', 'step':'0.001', 'answer_field': True}))
...
class PedigreeAnalyserForm(forms.Form):
    INH_PATTERN_CHOICES = [('-1', '-- Choose inheritance pattern --'),('AR', 'Autosomal Recesssive'), ('AD', 'Autosomal Dominant'), ('XR', 'X-Linked Recesssive'), ('XD', 'X-Linked Dominant'),  ('YR', 'Y-Linked') ]
